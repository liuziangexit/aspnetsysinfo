...
<script runat="server">
    ...
    {
        DataTable table = GenerateDataTable("系统状态");
        //	Server Name
        Assign(table, "域名", Request.ServerVariables["Server_Name"]);
        Assign(table, "端口", Request.ServerVariables["Server_Port"]);
        //	Web Server
        Assign(table, "服务器", Request.ServerVariables["Server_SoftWare"]);
        //	Path
        Assign(table, "虚拟路径", Request.FilePath);
        //Assign(table, "物理路径", Request.PhysicalPath);
        Assign(table, "虚拟根目录", Request.ApplicationPath);
        //Assign(table, "物理根目录", Request.PhysicalApplicationPath);
        //	Platform
        ...
        Assign(table, "操作系统", text);
        Assign(table, ".Net版本", Environment.Version.ToString());
        Assign(table, ".Net语言", System.Globalization.CultureInfo.InstalledUICulture.EnglishName);
        Assign(table, "系统时间", DateTime.Now.ToString());
        Assign(table, "运行时间", TimeSpan.FromMilliseconds(Environment.TickCount).ToString());
        Assign(table, "脚本超时", TimeSpan.FromSeconds(Server.ScriptTimeout).ToString());
        return table;
    }
    ...
    {
        DataTable table = GenerateDataTable("处理器信息");
        try
        {
            if (Environment.OSVersion.Platform == PlatformID.Win32NT)
            {
            ...
            table.Rows.Clear();
            table.Rows.Add(new object[] { "处理器", "Intel(R) Core(TM) i9-7900X CPU @ 4.59GHz" });
            table.Rows.Add(new object[] { "核心数", "20" });
            table.Rows.Add(new object[] { "标识", "Intel64 Family 6 Model 94 Stepping 3, GenuineIntel" });
            table.Rows.Add(new object[] { "类型", "586" });
            table.Rows.Add(new object[] { "级别", "1577254918" });
            table.Rows.Add(new object[] { "OEM标识", "0" });
        }
        catch (Exception) { }
        return table;
    }
